...
export default async function ShopPage({ searchParams }: { searchParams: Promise<{ category?: string; collection?: string }> }) {
  ...
  const [products, categories] = await Promise.all([getStorefrontProducts(category, collection), getStorefrontCategories()]);
  return (
    <>
      ...
      <div className="mx-auto max-w-[1320px] px-5 py-14 md:px-10 md:py-20">
        <div className="flex flex-wrap items-center justify-between gap-4 border-y border-ink/10 py-4">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-[11px] font-bold uppercase tracking-[.16em]">
            <Link href="/shop" className={!category && !collection ? "text-gold" : "text-ink/45 transition hover:text-ink"}>All</Link>
            {categories.map((item) => <Link key={item.id} href={`/shop?category=${item.slug}`} className={category === item.slug && !collection ? "text-gold" : "text-ink/45 transition hover:text-ink"}>{item.name}</Link>)}
          </div>
        </div>
        {products.length ? (
