...
import { FadeIn, StaggerContainer, StaggerItem } from "@/components/storefront/fade-in";
    ...
    <>
          ...
          <StaggerContainer className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {categories.slice(0, 6).map((category, index) => (
              <StaggerItem key={category.id}>
                <Link href={`/shop?category=${category.slug}`} className="group relative block aspect-[4/5] overflow-hidden" style={{ backgroundColor: categoryTones[index % categoryTones.length] }}>
                  {category.imageUrl ? (
      ...
      {arrivals.length > 0 && (
        <section className="border-t border-ink/10 bg-white/60 mx-auto max-w-[1320px] px-5 py-20 md:px-10 md:py-28">
          ...
          <StaggerContainer className="mt-12 grid grid-cols-2 gap-x-4 gap-y-12 md:grid-cols-3 md:gap-6 lg:grid-cols-4">
      ...
      <section className="bg-sand">
        <FadeIn className="mx-auto grid max-w-[1320px] items-center gap-12 px-5 py-20 md:grid-cols-2 md:px-10 md:py-28">
      ...
      <section className="mx-auto max-w-[1320px] px-5 py-20 md:px-10 md:py-28">
        ...
        <StaggerContainer className="mt-14 grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-5">
          {whyItems.map((item) => (
            <StaggerItem key={item.title} className="border-t border-ink/15 pt-6 text-center sm:text-left">
